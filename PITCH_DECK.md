...
> **Local Backup:** `http://localhost:5173` (Vite) + `http://localhost:4000` (Server)

---

## 📋 SLIDE DECK STRUCTURE AT A GLANCE

| Slide # | Slide Title | Core Theme | Time Target |
| :--- | :--- | :--- | :--- |
| **01** | **Title & Vision Hook** | Brand identity, cyber-strategy esports platform | 0:00 – 0:30 |
| **02** | **The Problem Space** | Client tampering, asset bloat, shallow tactics | 0:30 – 1:00 |
...
## 🖥️ SLIDE-BY-SLIDE CONTENT & SPEAKER SCRIPTS
...
- Badges: `SERVER-AUTHORITATIVE` • `PROCEDURAL AUDIO` • `REVERSE ENGINEERING ENGINE` • `100% REGRESSION VERIFIED`
...
> We built NEXORA from the ground up to prove that browser gaming can deliver the adrenaline, precision, acoustic intensity, and visual spectacle of an MNC/AAA esports title — without multi-gigabyte downloads and without trusting a single line of client code."*
...
> And third, **lack of intellectual depth**. There hasn't been a game that bridges high-level esports strategy with genuine computer science engineering mechanics."*
...
- Comparison Cards:
  - 🛠 **FORWARD ENGINEERING (The Net-Architect & Cyber Sentinel)**
    ...
    - Node overclocking (+50 PTS)
    - Multi-layered hexagonal rotating forcefields
    - Network topology expansion
  - 🔓 **REVERSE ENGINEERING (The Cryptanalyst & Ghost Operative)**
    - Real-time cryptographic frequency decompile minigame
    - Neutralizing fortified hostile shields in a 10-second hacking window
    - Packet siphoning (+25 extra PTS on breach)
...
> It rewards tactical foresight, spatial planning, and analytical reflexes."*
...
- Tech Badge Grid:
  ...
  - **Visual FX**: Hardware-Accelerated 2D HTML5 Canvas -> *60 FPS conduits & shockwaves*
  - **Tactical Radar**: Contextual right-click MOBA pings with expanding radar rings
...
- Key demo choreography:
  1. Instant Matchmaking via live radar sweep
  2. Grid synchronization & turn announcer callout
  3. Matrix-green adjacency guides & legal moves
  4. Core node capture (+200 PTS & screen shake)
  5. Deploying a Firewall Shield -> Opponent triggering the Reverse Engineering Decompile Minigame
...
> Both enter the queue. The radar sweep searches, pairs them based on ELO rating, and immediately establishes a synchronized socket session.  
> Listen to the announcer: 'Neural combat link established. Welcome to the CyberGrid.'  
> As Operative 1, my legal Manhattan adjacent nodes glow in matrix-green. I advance — hear the procedural acoustic chirp, watch the circuit conduit form on the canvas.  
> Now Operative 2 deploys an energy shield — notice the 60 FPS rotating hexagonal forcefield.  
> Back on Operative 1, the hostile node is shielded. I activate the **Reverse Engineer Decompilation protocol**. The memory dump opens — I adjust the cryptographic frequency registers to match the target. Done! The firewall collapses, triggering a camera shake and breach fanfare!  
> I can also right-click anywhere to drop tactical MOBA radar beacons for attack, defend, or scan."*
...
- Screenshot or live view of `/replay/:matchId`:
  ...
  - Historical turn-by-turn state reconstruction from PostgreSQL audit events
...
> Post-game, our server calculates ELO rating adjustments using our authoritative ranking algorithm, updating player streaks, win rates, and global podium standings in real time."*
...
- Architecture Security Diagram:
  ...
  - **State Versioning**: Monotonic state versions (`v1`, `v2`, `v3`) rejecting stale client submissions
  - **Rule-Based Anomaly Engine**: Bounded 0–100 anomaly scoring identifying suspicious activity
...
- Multi-Cloud Deployment Topology:
  ...
  - **Database**: Managed PostgreSQL with connection pooling and automated schema migrations.
...
- Summary Pillars:
  ...
  - 🛡 **Integrity**: 100% server-authoritative engine with real-time anomaly auditing.
  - 🌐 **Live URL**: `https://nexora-grid-9024.web.app`
...
## 🎬 3-MINUTE HACKATHON LIVE DEMO CHOREOGRAPHY

1. **Step 1 (0:00 - 0:30) — The Setup**:
   - Have two windows open side-by-side at `https://nexora-grid-9024.web.app` (or `localhost:5173`).
   - Click **LOGIN** -> Click **OPERATIVE 1** in Left Window, **OPERATIVE 2** in Right Window.
   - Mention: *"One-click demo credentials for instant evaluation."*

2. **Step 2 (0:30 - 1:15) — The Matchmaking Radar**:
   - Click **FIND MATCH** on both screens.
   - Show the circular radar sweep and live queue timer.
   - Watch the head-to-head match found banner fire with robotic announcer callout.

3. **Step 3 (1:15 - 2:00) — Tactical Grid & Canvas Conduits**:
   - Turn 1: Left player moves to adjacent neutral node (`N01`).
   - Point out: Matrix-green adjacency guides, procedural capture chord, and dynamic canvas circuit conduit forming between nodes.

4. **Step 4 (2:00 - 2:30) — Defense & Reverse Engineering Decompile**:
   - Right player moves and presses **DEFEND**. Point out the 60 FPS rotating hexagonal shield.
   - Left player targets the shielded node and clicks **REVERSE ENGINEER (DECOMPILE HOSTILE SHIELD)**.
   - Quickly align the 3 numbers to crack the code. Show the camera shake and shield breach!

5. **Step 5 (2:30 - 3:00) — Esports Replay & Audit Log**:
   - Complete the match or click **ENGAGEMENT HISTORY** -> Click **REPLAY ▶**.
   - Show the timeline scrubber, speed multiplier (`2x`), and explain that every turn was reconstructed from server audit logs!
